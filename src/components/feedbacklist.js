...
function FeedBackList() {
    ...
    const fetchData = async () => {
      ...
      try {
        const loggedRes = await fetch(
          `http://localhost:5000/loggedFeedback?email=${sessionStorage.getItem("user_email")}`
        );
        const feedbackRes = await fetch(`http://localhost:5000/feedBacklist?loggedEmail=${sessionStorage.getItem("user_email")}`);
      ...
      const totalRequest=await fetch(`http://localhost:5000/totalFeedbacks?email=${sessionStorage.getItem("user_email")}`);
      if(totalRequest.ok) {
  ...
  const deleteFeedback = async () => {
    ...
    setLogFeedback([]);
    try {
      const reqs = await fetch("http://localhost:5000/deleteFeedback", {
        method: "DELETE",
        headers: {
  ...
  const handleShowmore=async(rating)=>{ //When show more is clicked for a rating
    ...
    try{
      ...
      const reqs=await fetch(`http://localhost:5000/showMorefeedbacks?email=${email}&currentNum=${num}&rating=${rating}`);
